...
math::float3 radiance(math::float3 ray) {
    constexpr math::float3 horizon{.65f, .72f, .80f};
    constexpr math::float3 zenith{.20f, .32f, .48f};
    constexpr math::float3 ground{.16f, .15f, .12f};
    const float t = std::pow(std::abs(ray.y), ray.y >= 0 ? .45f : .35f);
...
struct environment_lighting::impl {
    Engine& engine;
    Scene& scene;
    Texture* dome{};
    Texture* reflections{};
...
environment_lighting::environment_lighting(Engine& engine, Scene& scene)
...
environment_lighting::~environment_lighting() = default;
}
